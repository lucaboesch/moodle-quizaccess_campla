...
Feature: CAMPLA button in quiz edit form
  ...
  Scenario Outline: Pushing the "Generate CAMPLA configuration" button.
    ...
    Examples:
      | timeopen              | timeclose             |
      | ##31 Dec 2039 12:00## | ##31 Dec 2040 12:00## |

  Scenario: The "Generate CAMPLA configuration" button should be enabled or disabled based on quiz timing.
    Given the following "activities" exist:
      | activity | course | section | name   |
      | quiz     | C1     | 1       | Quiz 1 |
    When I am on the "Quiz 1" "quiz activity editing" page logged in as admin
    And I expand all fieldsets
    Then the "Generate CAMPLA configuration" "button" should be disabled
    And I should see "Save the changes for them to take effect."
    And I expand all fieldsets
    ...
    And I expand all fieldsets
    And I should see "Both this dates have to be in the future."
    And I expand all fieldsets
    And I set the following fields to these values:
      | timeopen[enabled] | 1 |
      | timeopen[day] | 1 |
    ...
    And I press "Save and return to course"
    And I am on the "Quiz 1" "quiz activity editing" page logged in as admin
    And the "Generate CAMPLA configuration" "button" should be enabled
